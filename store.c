...
void dndstore(void)
{
	...
	for (;;)
	{
		...
		if (i == 12) {
			...
			else if (cdesc[GOLD] < (dnd_item[i].price) * 10L) nogold(); 
			else {
...
void oschool(void)
{
	...
	for (;;)
	{
		cursor(57,18); lprintf("%d gold pieces.   ",(int)cdesc[GOLD]); cursors();
		...
		{ drawscreen(); return; }
		lprc((char)i);
		if (cdesc[GOLD] < 250)  nogold();  else
			if (course[i-'a'])
			{ lprcat("\nSorry, but that class is filled."); nap(NAPTIME); }
			else
				if (i <= 'h')
				{
					cdesc[GOLD] -= 250; time_used=0;
					switch(i)
					{
					case 'a':   cdesc[STRENGTH] += 2;  cdesc[CONSTITUTION]++;
						lprcat("\nYou feel stronger!");
						cl_line(16,7);
						break;
					...
					case 'b':   if (course[0]==0)
								{
									...
									cdesc[GOLD]+=250;  time_used= -10000;  break;
								}
								lprcat("\nYou feel much stronger!");
								cl_line(16,8);
								cdesc[STRENGTH] += 2;  cdesc[CONSTITUTION] += 2;  break;

					case 'c':   cdesc[INTELLIGENCE] += 2; 
					...
					case 'd':   if (course[2]==0)
								...
								cl_line(16,10);
								cdesc[INTELLIGENCE] += 2;  break;

					case 'e':   cdesc[CHARISMA] += 3;  
						lprcat("\nYou now feel like a born leader!");
						cl_line(16,11);  break;

					case 'f':   cdesc[WISDOM] += 2;  
						lprcat("\nYou now feel more confident that you can find the potion in time!");
						cl_line(16,12);  break;

					case 'g':   cdesc[DEXTERITY] += 3;  
						lprcat("\nYou feel like dancing!");
						cl_line(16,13);  break;

					case 'h':   cdesc[INTELLIGENCE]++;
					...
					if (time_used > 0)
					{
						...
						cdesc[HP] = cdesc[HPMAX];  cdesc[SPELLS] = cdesc[SPELLMAX]; /* he regenerated */

						if (cdesc[BLINDCOUNT])    cdesc[BLINDCOUNT]=1;  /* cure blindness too!  */
						if (cdesc[CONFUSE])       cdesc[CONFUSE]=1;   /*  end confusion   */
...
void ointerest(void)
{
	...
	if (cdesc[BANKACCOUNT]<0) cdesc[BANKACCOUNT] = 0;
	else if ((cdesc[BANKACCOUNT]>0) && (cdesc[BANKACCOUNT]<500000))
	{
		...
		if (cdesc[BANKACCOUNT]>500000) cdesc[BANKACCOUNT]=500000; /* interest limit */
	}
...
static void obanksub(void)
{
	...
	cursor(31,17); lprintf("You have %8d gold pieces in the bank.",(int)cdesc[BANKACCOUNT]);
	cursor(40,18); lprintf("You have %8d gold pieces",(int)cdesc[GOLD]);
	...
	for (;;)
	{
		...
		case 'd':
			lprcat("deposit\n");
			cltoeoln();
			lprcat("How much? "); amt = readnum((int)cdesc[GOLD]);
			if (amt>cdesc[GOLD])
			{ lprcat("  You don't have that much.");  nap(NAPTIME); }
			else { cdesc[GOLD] -= amt;  cdesc[BANKACCOUNT] += amt; }
			break;

		case 'w':   lprcat("withdraw\nHow much? "); amt = readnum((int)cdesc[BANKACCOUNT]);

			if (amt > cdesc[BANKACCOUNT])
			{ lprcat("\nYou don't have that much in the bank!"); nap(NAPTIME); }
			else { cdesc[GOLD] += amt;  cdesc[BANKACCOUNT] -= amt; }
			break;
		...
		case 's':   lprcat("\nWhich stone would you like to sell? ");
			...
			if (i=='*')
			{
				for (i=0; i<26; i++)
				{
					...
					{
						gems_sold = TRUE ;
						cdesc[GOLD]+=gemvalue[i];  iven[i]=0;
		...
		cursor(40,17); lprintf("%8d",(int)cdesc[BANKACCOUNT]);
		cursor(49,18); lprintf("%8d",(int)cdesc[GOLD]);
	}
}
			...
			if (iven[isub]==OSCROLL)
				...
				if (iven[isub]==OPOTION)
					...
					*/
					...
					{
						lprcat("yes\n"); cdesc[GOLD]+=value;
						if (cdesc[WEAR] == isub) cdesc[WEAR] = -1;
						if (cdesc[WIELD] == isub) cdesc[WIELD] = -1;
						if (cdesc[SHIELD] == isub) cdesc[SHIELD] = -1;
