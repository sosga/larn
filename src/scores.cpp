...
paytaxes ( int x )
{
	...
	for ( i = 0; i < SCORESIZE; i++ )
		if ( strcmp ( winr[i].who,
		              logname ) == 0 )	/* look for players winning entry */
			if ( winr[i].score >
...
winshou ( void )
{
	...
	for ( count = j = i = 0; i < SCORESIZE;
	      i++ )	/* is there anyone on the scoreboard? */
		if ( winr[i].score != 0 )
		{
			j++;
			break;
...
showallscores ( void )
{
	...
	cdesc[WEAR] = cdesc[WIELD] = cdesc[SHIELD] =
...
invalid:
	...
	if ( ( wizard == 0 )
	     && ( cdesc[GOLD] > 0 ) )  	/*  wizards can't score     */
	{
...
diedsub ( int x )
{
	const char *mod;
	char ch;
	lprintf ( "Score: %ld, Diff: %ld,  %s ", cdesc[GOLD],
	          cdesc[HARDGAME],
	          logname );
...
diedlog ( void )
{
	...
	for ( n = stbuf.st_size / sizeof ( struct log_fmt ); n > 0;
	      --n )
	{
		...
		lprintf ( "Score: %d, Diff: %d,  %s %s on %d at %s",
		          ( int ) ( logg.score ),
		          ( int ) ( logg.diff ), logg.who, logg.what,
		          ( int ) ( logg.cavelev ),
		          p + 4 );
