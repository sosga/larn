...
act_donation_pray (void)
{
	...
	for (;;) {
		...
		if (cdesc[GOLD] >= k) {
			...
			if (k < temp || k < rnd (50)) {
				/* added by ~Gibbon */
				printw ("You have offended the Gods.");
			...
			if (rnd (43) == 5) {
				if (cdesc[WEAR])
					printw ("You feel your armor vibrate for a moment");
				enchantarmor ();
				return;
			}
			if (rnd (43) == 8) {
				if (cdesc[WIELD])
					printw ("You feel your weapon vibrate for a moment");
				enchweapon ();
				return;
			}
			printw ("Thank You.");
