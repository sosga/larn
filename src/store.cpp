...
dnd_2hed (void)
{
	...
	printw ("Also be advised, if you break 'em, you pay for 'em.");
}
...
sch_hed (void)
{
	...
	printw ("\t\t    Course Name \t       Time Needed\n\n");
	if (course[0] == 0)
		printw ("\t\ta)  Fighters Training I         10 mobuls");	/*line 7 of crt */
	lprc ('\n');
	if (course[1] == 0)
		printw ("\t\tb)  Fighters Training II        15 mobuls");
	lprc ('\n');
	if (course[2] == 0)
		printw ("\t\tc)  Introduction to Wizardry    10 mobuls");
	lprc ('\n');
	if (course[3] == 0)
		printw ("\t\td)  Applied Wizardry            20 mobuls");
	lprc ('\n');
	if (course[4] == 0)
		printw ("\t\te)  Behavioral Psychology       10 mobuls");
	lprc ('\n');
	if (course[5] == 0)
		printw ("\t\tf)  Faith for Today             10 mobuls");
	lprc ('\n');
	if (course[6] == 0)
		printw ("\t\tg)  Contemporary Dance          10 mobuls");
	lprc ('\n');
	if (course[7] == 0)
		printw ("\t\th)  History of Larn              5 mobuls");
	printw ("\n\n\t\tAll courses cost 250 gold pieces.");
	cursor (1, 19);
	printw ("You are presently carrying ");
}
...
banktitle (char *str)
{
	...
	if (outstanding_taxes > 0) {
		...
		printw ("We suggest you go to the LRS office and pay your taxes.\n");
		cursors ();
		printw ("\nPress ");
		lstandout ("escape");
		printw (" to leave: ");
...
ointerest (void)
{
	...
	if (cdesc[BANKACCOUNT] < 0)
		cdesc[BANKACCOUNT] = 0;
	else if ((cdesc[BANKACCOUNT] > 0)
	         && (cdesc[BANKACCOUNT] < 500000)) {
...
obanksub (void)
{
	...
	for (;;) {
		...
		case 'd':
			...
			if (amt > cdesc[GOLD]) {
				printw ("You don't have that much.");
				nap (NAPTIME);
			}
			/* Added if statement for catching 0 value to deposit. -Gibbon */
			if (amt == 0) {
				printw ("You cannot deposit nothing");
				nap (NAPTIME);
			} else {
				cdesc[GOLD] -= amt;
...
otradhead (void)
{
	...
	printw ("damaged, we will pay only 10% of their new value.\n\n");
	printw ("Here are the items we would be willing to buy from you:\n");
}
...
nxt:
		...
		if (outstanding_taxes > 0) {
			...
			printw ("You do not owe us any taxes.           ");
		}
		cursor (1, 19);
		if (cdesc[GOLD] > 0) {
			amtgoldtrad();
		} else {
			printw ("You have no gold pieces.  ");
		}
	}
}
