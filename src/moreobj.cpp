...
fntchange (int how)
{
	...
	switch (rnd (9)) {
	case 1:
		printw ("Your strength");
		fch (how, &cdesc[STRENGTH]);
		break;
	case 2:
		printw ("Your intelligence");
	...
	case 3:
		...
		break;
	case 4:
		printw ("Your constitution");
		fch (how, &cdesc[CONSTITUTION]);
		break;
	case 5:
		printw ("Your dexterity");
...
{
	if (how < 0) {
		printw (" went down by one!");
		--(*x);
	} else {
		printw (" went up by one!");
		(*x)++;
	}
	bottomline ();
}
...
open_something (void)
{
	...
	if (item[playerx][playery] == OCHEST) {
		printw ("There is a chest here.  Open it?");
		if ((tempc = getyn ()) == 'y') {
			act_open_chest (playerx, playery);
			dropflag =
	...
	switch (item[x][y]) {
	case OOPENDOOR:
		printw ("The door is already open!");
		break;
	case OCHEST:
		act_open_chest (x, y);
		break;
...
close_something (void)
{
	...
	switch (item[x][y]) {
	case OCLOSEDDOOR:
		printw ("The door is already closed!");
		break;
	case OOPENDOOR:
		if (mitem[x][y]) {
			printw ("Theres a monster in the way!");
			return;
		}
