...
static void speldamage(int x)
{
	...
	if (cdesc[TIMESTOP])  { lprcat("  It didn't seem to work"); return; }  /* not if time stopped */
	clev = cdesc[LEVEL];
	...
	case 0: if (cdesc[PROTECTIONTIME]==0)   cdesc[MOREDEFENSES]+=2; /* protection field +2 */
		cdesc[PROTECTIONTIME] += 250;   return;
	...
	case 2: if (cdesc[DEXCOUNT]==0) cdesc[DEXTERITY]+=3; /* dexterity   */
		cdesc[DEXCOUNT] += 400;     return;
...
ws: 
	...
	case 4: /*  charm monster   */  cdesc[CHARMCOUNT] += cdesc[CHARISMA]<<1;    return;
	...
	case 7: if (cdesc[STRCOUNT]==0) cdesc[STREXTRA]+=3; /*  strength    */
		cdesc[STRCOUNT] += 150+rnd(100);    return;
	...
	case 10:    cdesc[BLINDCOUNT]=0;    return; /* cure blindness   */

	case 11:    createmonster(makemonst(level+1)+8);  return;

	case 12:    if (rnd(11)+7 <= cdesc[WISDOM]) direct(x,rnd(20)+20+clev,"  The %s believed!",0);
	...
	case 13:    /* if he has the amulet of invisibility then add more time */
		...
		cdesc[INVISIBILITY] += (j<<7)+12;   return;
	...
	case 17:    cdesc[CANCELLATION]+= 5+clev;   return; /*  cancellation    */

	case 18:    cdesc[HASTESELF]+= 7+clev;  return;  /* haste self  */
	...
	case 20:    xh = min(playerx+1,MAXX-2);     yh = min(playery+1,MAXY-2);
		for (i=max(playerx-1,1); i<=xh; i++) /* vaporize rock */
			for (j=max(playery-1,1); j<=yh; j++)
			{
				...
				case OSTATUE: if (cdesc[HARDGAME]<3)
							  {
	...
	case 23:    i=min(cdesc[HP]-1,cdesc[HPMAX]/2);  /* drain life */
		direct(x,i+i,"",0); cdesc[HP] -= i;     return;

	case 24:    if (cdesc[GLOBE]==0) cdesc[MOREDEFENSES] += 10;
		cdesc[GLOBE] += 200;  loseint();  /* globe of invulnerability */
		return;

	case 25:    omnidirect(x,32+clev,"  The %s struggles for air in your flood!"); /* flood */
		return;

	case 26:    if (rnd(151)==63) { lprcat("\nYour heart stopped!\n"); nap(NAPTIME);  died(270); return; }
				if (cdesc[WISDOM]>rnd(10)+10) direct(x,2000,"  The %s's heart stopped",0); /* finger of death */
				else lprcat("  It didn't work"); return;

				/* ----- LEVEL 5 SPELLS ----- */

	case 27:    cdesc[SCAREMONST] += rnd(10)+clev;  return;  /* scare monster */

	case 28:    cdesc[HOLDMONST] += rnd(10)+clev;  return;  /* hold monster */

	case 29:    cdesc[TIMESTOP] += rnd(20)+(clev<<1);  return;  /* time stop */
	...
	case 37:    /* permanence */ adjtimel(-99999L);  spelknow[37]=0; /* forget */
		loseint();
		return;

	default:    lprintf("  spell %d not available!",(int)x); return;
	};
}
...
static int isconfuse(void)
{
	if (cdesc[CONFUSE]) { lprcat(" You can't aim your magic!"); }
...
fool:
			arg += 2;
			while (arg-- > 0) { parse2(); nap(NAPTIME); }
			return;
		}
		else if (spnum==6) /* web */
		{
	...
	lprintf(str,lastmonst,(int)arg);       hitm(x,y,dam);
}
