...
void regen(void)
{
    ...
    if (d[TIMESTOP])
        {
            if(--d[TIMESTOP]<=0) bottomline();     /* for stop time spell */
            return;
        }
    flag=0;
    ...
    if (d[HERO])            if (--d[HERO]<=0)
            {
                for (i=0; i<6; i++) d[i] -= 10;
                flag=1;
            }
    if (d[ALTPRO])          if (--d[ALTPRO]<=0)
            {
                d[MOREDEFENSES]-=3;
                flag=1;
            }
    if (d[PROTECTIONTIME])  if (--d[PROTECTIONTIME]<=0)
            {
                d[MOREDEFENSES]-=2;
                flag=1;
            }
    if (d[DEXCOUNT])        if (--d[DEXCOUNT]<=0)
            {
                d[DEXTERITY]-=3;
                flag=1;
            }
    if (d[STRCOUNT])        if (--d[STRCOUNT]<=0)
            {
                d[STREXTRA]-=3;
                flag=1;
            }
    if (d[BLINDCOUNT])      if (--d[BLINDCOUNT]<=0)
            {
                cursors();
                lprcat("\nThe blindness lifts  ");
            }
    if (d[CONFUSE])         if (--d[CONFUSE]<=0)
            {
                cursors();
                lprcat("\nYou regain your senses");
            }
    if (d[GIANTSTR])        if (--d[GIANTSTR]<=0)
            {
                d[STREXTRA] -= 20;
                flag=1;
            }
